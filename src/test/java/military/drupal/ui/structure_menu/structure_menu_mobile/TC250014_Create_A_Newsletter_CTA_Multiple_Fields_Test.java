...
public class TC250014_Create_A_Newsletter_CTA_Multiple_Fields_Test extends AdminUITestBase {
    ...
    @Test(dataProvider = "setNLSignUpData")
    @Priority(level = PriorityLevel.HIGH)
    @Attributes(attributes = {@Attribute(key = "TestCaseId", value = "250014")})
    @TestCaseId("250014")
    public void createANewsletterCTAMultipleFields(List<ISectionDataModel> nlSignUpData, List<ISectionDataModel> landingPageData) {
        ...
        logStep("Select Call to Action - NL Signup");
        ...
        logStep("In the Form section, confirm 'Multiple Fields' is listed in the Displayed Fields field");
        logStep("Type Test Content in the Button Label field");
        logStep("Add Test Content in the ISRC Field");
        logStep("Select Newsletters in the Target Newsletters field");
        logStep("Click Save");
        logStep("Verify Drupal CTA NL Sign up created " + Configuration.baseUrl + "admin/structure/eck/entity/call_to_action/###");
        ...
        CTABaseModel ctaBaseModel = nlSignUpData.stream()
                                                .filter(d -> d instanceof CTABaseModel)
                                                .map(c -> (CTABaseModel) c)
                                                .findFirst()
                                                .get();
        CTAButtonModel ctaButtonModel = nlSignUpData.stream()
                                                    .filter(d -> d instanceof CTAButtonModel)
                                                    .map(c -> (CTAButtonModel) c)
                                                    .findFirst()
                                                    .get();
        addCallToActionNLSignupPage.fillSignupCTA(nlSignUpData)
                                   .clickSaveButton()
    ...
    @DataProvider
    public Object[][] setNLSignUpData() {
        return new Object[][]{
                {
                        List.of(
                                CTABaseModel.getNLSignupMultipleFieldsData(),
                                NLSignupFormModel.fillMultipleFieldsData(),
