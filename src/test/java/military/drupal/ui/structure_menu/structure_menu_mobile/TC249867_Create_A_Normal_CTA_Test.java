...
import drupal.ui.pages.structure.eck_entity_types.call_to_action.AddCallToActionNormalPage;
...
import drupal.ui.pages.structure.eck_entity_types.call_to_action.CTANormalModel;
...
public class TC249867_Create_A_Normal_CTA_Test extends AdminUITestBase {

    private UserModel user;

    @BeforeMethod(alwaysRun = true, inheritGroups = false)
    public void preconditionCreateANormalCTA() {
        user = loadUserModel("MILITARY_TEST_QA_USER");
    }

    @Test(dataProvider = "callTOActionData")
    @Priority(level = PriorityLevel.HIGH)
    @Attributes(attributes = {@Attribute(key = "TestCaseId", value = "249867")})
    @TestCaseId("249867")
    public void createANormalCTA(List<ISectionDataModel> normalCTAData, List<ISectionDataModel> landingPageData) {
        ...
        AddCallToActionNormalPage addCallToActionNormalPage = addCallToActionContentPage.clickCallToActionItem(AddCallToActionContentPage.CallToActionItem.CALL_TO_ACTION_NORMAL, AddCallToActionNormalPage.class);

        logStep("Enter test name in the Internal Title field");
        logStep("Enter test content in the Title field");
        logStep("Select Large in the Title Size field");
        logStep("Type Test content in the Blurb field");
        logStep("Type Test URL in the Link field");
        logStep("Type Test URL in the Tracking Pixel field");
        ...
        logStep("Type 'Desktop CTA' in the Media Name field and click SEARCH");
        logStep("Select one of the images and click SELECT IMAGES");
        logStep("Click Save");
        CTABaseModel ctaBaseData = normalCTAData.stream()
                                                .filter(d -> d instanceof CTABaseModel)
                                                .map(c -> (CTABaseModel) c)
                                                .findFirst()
                                                .get();
        CTAButtonModel ctaButtonData = normalCTAData.stream()
                                                    .filter(d -> d instanceof CTAButtonModel)
                                                    .map(c -> (CTAButtonModel) c)
                                                    .findFirst()
                                                    .get();
        addCallToActionNormalPage.fillIn(normalCTAData)
        ...
        return new Object[][]{
                {
                        List.of(
                                new CTABaseModel(),
                                new CTAButtonModel(),
                                CTAImageModel.getNormalCTAImage(),
                                new CTANormalModel()
                        ),
                        List.of(
