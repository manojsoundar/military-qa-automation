...
public class TC249794_Create_Content_Taxonomy_Category_Sub_Channel_Test extends AdminUITestBase {

    private UserModel user;

    @BeforeMethod(alwaysRun = true, inheritGroups = false)
    public void preconditionCreateContentTaxonomyCategorySubChannel() {
        user = loadUserModel("MILITARY_TEST_QA_USER");
    }

    @Test(dataProvider = "categoryAddTerm")
    @Priority(level = PriorityLevel.HIGH)
    @Attributes(attributes = {@Attribute(key = "TestCaseId", value = "249794")})
    @TestCaseId("249794")
    public void createContentTaxonomyCategorySubChannel(List<ISectionDataModel> landingPageData, List<ISectionDataModel> taxonomyCategoriesData) {
        ...
        logStep("Click Relations, type and select History");
        logStep("Click Save");
        logStep("Validation : Drupal Taxonomy URL " + Configuration.baseUrl + "/taxonomy/term/#####");
        CategoriesTitleModel categoriesTitleModel = taxonomyCategoriesData.stream()
                                                                          .filter(d -> d instanceof CategoriesTitleModel)
                                                                          .map(c -> (CategoriesTitleModel) c)
                                                                          .findFirst()
                                                                          .get();
        taxonomyCategoriesData.add(new CategoriesUrlModel(null, linkUrl));
        CategoriesTermLandingPage categoriesTermLandingPage = categoriesAddTermPage.addNewTerm(taxonomyCategoriesData)
                                                                                   .clickSaveButton()
    ...
    public Object[][] categoryAddTerm() {
        return new Object[][]{
                {
                        List.of(AddLandingPageModel.taxonomyCategorySubChannelPreCondition(), new ThumbnailImageModel("Test Image")),
                        new ArrayList<>(
                                Arrays.asList(
                                        new CategoriesTitleModel("Test Equipment Category" + timeStampFormat(PATTERN)),
