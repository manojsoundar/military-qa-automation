...
public class Create_Edit_Delete_Sidebar_Components_Add_Listview_Test extends AdminUITestBase {
    ...
    public void createSidebarWithBlock(List<ISectionDataModel> sidebarData, List<ISectionDataModel> landingPageData, List<ISectionDataModel> flexibleEditData) {
        ...
        AddLandingPageModel editLandingPageData = flexibleEditData.stream()
                                                                  .filter(d -> d instanceof AddLandingPageModel)
                                                                  .map(c -> (AddLandingPageModel) c)
                                                                  .findFirst()
                                                                  .get();
        ...
        addLandingPage.fillIn(landingPageData)
                      .clickSaveButton()
                      .validateLandingPage(editLandingPageData);
    }
        ...
        return new Object[][]{
                {
                        List.of(
                                ...
                                new FlexibleContentSpaceBlockModel(CONTENT, 1, "Block Title " + timeStampFormat(PATTERN), "AppNexus", "AppNexus", false, "Top - 728x90, 970x90, Brandscape Top")
                        ),
                        List.of(
                                AddLandingPageModel.getSideBarBlockData()
                        ),
                        List.of(
                                AddLandingPageModel.getSideBarBlockData(),
                                new FlexibleSpaceBaseModel("Edit Test Title " + timeStampFormat(PATTERN)),
                                new FlexibleContentSpaceBlockModel(CONTENT, 0, "Edit Block Title " + timeStampFormat(PATTERN), "AppNexus", "AppNexus", false, "Top - 728x90, 970x90, Brandscape Top"),
                                new FlexibleContentSpaceBlockModel(CONTENT, 1, "Edit Block Title " + timeStampFormat(PATTERN), "AppNexus", "AppNexus", false, "Top - 728x90, 970x90, Brandscape Top")
                        ),
                }
        };
    }
