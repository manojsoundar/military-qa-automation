...
import static drupal.models.ContentSearchModel.ArticleType.EVER_GREEN;
import static drupal.models.ContentSearchModel.ContentType.ARTICLE;
import static drupal.models.ContentSearchModel.PublishingStatus.UNPUBLISHED;
import static drupal.models.UserModel.loadUserModel;

public class TC250662_Edit_An_Article_Set_Publish_On_Date_Test extends AdminUITestBase {

    private UserModel user;

    @BeforeMethod(alwaysRun = true, inheritGroups = false)
    public void preconditionEditAnArticleSetPublishDate() {
        user = loadUserModel("MILITARY_TEST_QA_USER");
    }

    @Test(dataProvider = "articleData")
    @Priority(level = PriorityLevel.HIGH)
    @Attributes(attributes = {@Attribute(key = "TestCaseId", value = "250662")})
    @TestCaseId("250662")
    public void editAnArticleSetPublishOnDate(List<ISectionDataModel> articleData, List<ISectionDataModel> editArticleData, ContentSearchModel searchData) {

        logStep("Login to Drupal");
        logStep("Enter Username and Password");
        DrupalHomePage homePage = new DrupalLoginPage().militaryLogin(user);

        logStep("Create an Evergreen article with un-publish status");
        ...
        logStep("In the Search field: Type Test Article in the Title field, Select UnPublished in the Published Status field and Select Evergreen in the Article type field");
        ...
        logStep("Enter today's date in the 'Publish On' input field in mm/dd/yyyy format.");
        logStep("Enter a time 2 minutes in the future in the 'Publish On' time input field in HH:MM:SS AM/PM format. Time should be entered relative to current time in US Pacific timezone");
        logStep("CLick Save");
        logStep("Verify: page displays the messages: 'Article <article title> has been updated'");
        logStep("Verify: message '<article title> is scheduled to be published <date entered in step 8> <time entered in step 9>'");
        logStep("Navigate to URL path /admin/content/scheduled");
        logStep("Verify the article schedule to be published appears in the list of scheduled content.");
        ...
        editArticlePage.editArticle(editArticleData)
                       ...
                       .clickOnPrimaryTab(ContentPage.PrimaryTabItem.SCHEDULED, ScheduledPage.class)
        ...
        return new Object[][]{
                {
                        ...
                        List.of(
                                SchedulingOptionsModel.getPublishData()
                        ),
                        new ContentSearchModel("Test Article", ARTICLE, UNPUBLISHED, EVER_GREEN)
                }
        };
    }
